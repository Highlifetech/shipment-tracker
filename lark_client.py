...
class LarkClient:
    ...
    def read_tracking_data(self, spreadsheet_token, sheet_id):
        start_row = HEADER_ROW + 1
        rows = self.read_sheet_range(spreadsheet_token, sheet_id,
                                     start_col="A", end_col="Q",
                                     start_row=start_row, end_row=500)
        col_idx = {
            "shipment_id": 0, "vendor": 1, "recipient": 2, "order_num": 3,
            "customer": 4, "tracking_num": 6, "carrier": 7, "status": 12,
            "delivery_date": 16,
        }
        results = []
        for i, row in enumerate(rows):
    ...
    def _build_card_message(self, text_content):
        ...
        return json.dumps(card)
    def send_daily_summary(self, all_results):
        """Send daily summary.
        Format: tracking_number -- recipient -- estimated delivery
        Dedup same tracking numbers. Brendan -> Brendan. Customer Direct -> customer name.
        """
        active = [r for r in all_results if r.get("new_status", "").upper() != "DELIVERED"]
        if not active:
            self.send_group_message("All shipments delivered. Nothing to track today.")
